...
class Bech32Encoder extends Converter<Bech32, String> with Bech32Validations {
  ...
  String convert(Bech32 input,
    ...
    if (hrp.length +
      ...
      throw '${Bech32Exception.tooLongBech32.message} ${hrp.length + data.length + 1 + Bech32Validations.checksumLength}';
    }

    if (hrp.isEmpty) {
      throw Bech32Exception.tooShortHrp.message;
    }

    if (hasOutOfRangeHrpCharacters(hrp)) {
      throw Bech32Exception.outOfRange.message;
    }

    if (isMixedCase(hrp)) {
      throw Bech32Exception.mixedCases.message;
    }

    hrp = hrp.toLowerCase();
    final convertedBytes = BitListCompressor.convert(data, 8, 5, true);

    var checksummed = convertedBytes + _createChecksum(hrp, convertedBytes);

    if (hasOutOfBoundsChars(checksummed)) {
      throw Bech32Exception.outboundChars.message;
    }
...
class Bech32Decoder extends Converter<String, Bech32> with Bech32Validations {
  ...
  Bech32 convert(String input,
    ...
    if (input.length > maxLength) {
      throw Bech32Exception.tooLongBech32.message;
    }
    ...
    var decodedData = data.split('').map((c) {
      return charset.indexOf(c);
    }).toList();

    final result = BitListCompressor.convert(decodedData, 5, 8, false);
    //Byte compression pass utint5 to uint

    if (hasOutOfBoundsChars(decodedData)) {
      throw '${Bech32Exception.outboundChars.message} ${data[decodedData.indexOf(-1)]}';
    }
    ...
    if (hasOutOfBoundsChars(checksumBytes)) {
      throw '${Bech32Exception.outboundChars.message} ${checksum[checksumBytes.indexOf(-1)]}';
    }

    if (isInvalidChecksum(hrp, decodedData, checksumBytes)) {
      throw Bech32Exception.invalidChecksum.message;
    }

    return Bech32(hrp, result);
  }
}
